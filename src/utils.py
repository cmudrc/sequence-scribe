...
def normalize_bbox(bbox, image_width, image_height):
    ...
    if any(isinstance(v, float) and np.isnan(v) for v in bbox):
        print(f"WARNING: NaN values in bbox -> {bbox}")
        return [0.0, 0.0, 0.0, 0.0]
...
def compute_iou(box1, box2):
    ...
    if any(isinstance(v, float) and np.isnan(v) for v in box1) or any(isinstance(v, float) and np.isnan(v) for v in box2):
        print(f"WARNING: NaN values in boxes for IoU computation -> {box1}, {box2}")
        return 0.0
    ...
    if model_name == "blip2":
        ...
        if device == 'cpu':
            model = AutoModelForCausalLM.from_pretrained(
                ...
                attn_implementation = 'eager' # Disables flash attention and persists default PyTorch attention mechanism
            )
        else:
            model = AutoModelForCausalLM.from_pretrained(
                model_name_or_path, 
...
def get_som_labeled_img(img_path, model=None, BOX_TRESHOLD = 0.01, output_coord_in_ratio=False, ocr_bbox=None, text_scale=0.4, text_padding=5, draw_bbox_config=None, caption_model_processor=None, ocr_text=[], use_local_semantics=True, iou_threshold=0.9,prompt=None, scale_img=False, imgsz=None, batch_size=None):
    ...
    for box in filtered_boxes:
        if box['content'] is not None:
            filtered_boxes_elem.append(box)
    for box in filtered_boxes:
